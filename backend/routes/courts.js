...
router.post('/', async (req, res) => {
    ...
    // Validate that sportId and centerId are valid MongoDB ObjectIds
    if (!mongoose.Types.ObjectId.isValid(sportId) || !mongoose.Types.ObjectId.isValid(centerId)) {
        return res.status(400).json({ message: 'Invalid sportId or centerId format' });
    }
